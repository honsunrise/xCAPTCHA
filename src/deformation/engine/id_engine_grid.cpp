...
Mat2d id_engine_grid::calc_delta(const int src_w,
  ...
  triangles = ::delaunayDiv(new_p, bound_rect);
  ...
  for (int i = 0;; i += grid_size) {
    ...
    for (int j = 0;; j += grid_size) {
      ...
      d0 = 1 - d1 - d2;
      delta(j, i) =
          Vec2d(d0 * delta(triangles[tId].v[0])[0] + d1 * delta(triangles[tId].v[1])[0]
                      + d2 * delta(triangles[tId].v[2])[0],
                  d0 * delta(triangles[tId].v[0])[1] + d1 * delta(triangles[tId].v[1])[1]
