...
class basic_config_define {
 ...
 public:
  ...
  using value_type = basic_config_define;
  ...
  using allocator_type = AllocatorType<basic_config_define>;

  using pointer = typename std::allocator_traits<allocator_type>::pointer;
  using const_pointer = typename std::allocator_traits<allocator_type>::const_pointer;

  using iterator = iter_impl<basic_config_define>;
  using const_iterator = iter_impl<const basic_config_define>;
  using reverse_iterator = conofig_reverse_iterator<typename basic_config_define::iterator>;
  using const_reverse_iterator = conofig_reverse_iterator<typename basic_config_define::const_iterator>;
  ...
  using map_t = ObjectType<StringType,
                           basic_config_define,
                           std::less<StringType>,
                           AllocatorType<std::pair<const StringType, basic_config_define>>>;


  using array_t = ArrayType<basic_config_define, AllocatorType<basic_config_define>>;
 ...
 public:
  basic_config_define(const value_t v)
      : m_type(v), m_value(v) {
    assert_invariant();
  }

  basic_config_define(std::nullptr_t = nullptr) noexcept
      : basic_config_define(value_t::null) {
    assert_invariant();
  }
  ...
  basic_config_define(initializer_list_t init,
                      bool type_deduction = true,
                      value_t manual_type = value_t::array) {
    // check if each element is an array with two elements whose first
    // element is a string
    bool is_an_object = std::all_of(init.begin(), init.end(),
                                    [](const detail::config_ref<basic_config_define> &element_ref) {
  ...
  static basic_config_define array(initializer_list_t init = {}) {
    return basic_config_define(init, false, value_t::array);
  }

  static basic_config_define object(initializer_list_t init = {}) {
    return basic_config_define(init, false, value_t::map);
  }

  basic_config_define(size_type cnt, const basic_config_define &val)
      : m_type(value_t::array) {
  ...
  template<class InputIT, typename std::enable_if<
      std::is_same<InputIT, typename basic_config_define::iterator>::value or
          std::is_same<InputIT, typename basic_config_define::const_iterator>::value, int>::type = 0>
  basic_config_define(InputIT first, InputIT last) {
  ...
  basic_config_define(basic_config_define &&other) noexcept
  ...
  reference &operator=(basic_config_define other) noexcept(
 ...
 public:
  ...
  template<class IteratorType, typename std::enable_if<
      std::is_same<IteratorType, typename basic_config_define::iterator>::value or
          std::is_same<IteratorType, typename basic_config_define::const_iterator>::value, int>::type
  = 0>
 ...
 public:
  ...
  reference operator+=(basic_config_define &&val) {
  ...
  reference operator+=(const basic_config_define &val) {
    push_back(val);
    return *this;
  }
  ...
  void push_back(initializer_list_t init) {
    if (is_map() and init.size() == 2 and (*init.begin())->is_string()) {
      basic_config_define &&key = init.begin()->moved_or_copied();
      push_back(typename map_t::value_type(
          std::move(key.get_ref<string_t &>()), (init.begin() + 1)->moved_or_copied()));
    } else {
      push_back(basic_config_define(init));
    }
  }
 ...
 public:
  ...
  friend bool operator==(const_reference lhs, const ScalarType rhs) noexcept {
    return (lhs == basic_config_define(rhs));
  }

  template<typename ScalarType, typename std::enable_if<
      std::is_scalar<ScalarType>::value, int>::type = 0>
  friend bool operator==(const ScalarType lhs, const_reference rhs) noexcept {
    return (basic_config_define(lhs) == rhs);
  }
 ...
 private:
  ...
  basic_config_define flatten() const {
    basic_config_define result(value_t::map);
    config_path::flatten("", *this, result);
    return result;
  }

  basic_config_define unflatten() const {
    return config_path::unflatten(*this);
  }
};

using config_define = basic_config_define<>;
