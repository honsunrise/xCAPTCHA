...
// overloads for basic_json template parameters
        ...
        void get_arithmetic_value(const BasicConfigType &j, ArithmeticType &val) {
            switch (static_cast<value_t>(j)) {
                ...
                default:
                    CONFIG_THROW(type_error::create(302, "type must be number, but is " + std::string(j.type_name())));
            }
        }
...
// forward_list doesn't have an insert method
        ...
        template<typename BasicConfigType, typename CompatibleArrayType>
        auto
        from_config_array_impl(const BasicConfigType &j, CompatibleArrayType &arr, utils::priority_tag<1> /*unused*/)
        -> decltype(
        ...
        template<typename BasicConfigType, typename T, std::size_t N>
        void
        from_config_array_impl(const BasicConfigType &j, std::array<T, N> &arr, utils::priority_tag<2> /*unused*/) {
...
// an arithmetic type?
        ...
        private:
            ...
            auto
            call(BasicConfigType &j, T &&val, utils::priority_tag<1> /*unused*/) const noexcept(noexcept(to_config(j,
                                                                                                                   std::forward<
                                                                                                                           T>(
                                                                                                                           val))))
